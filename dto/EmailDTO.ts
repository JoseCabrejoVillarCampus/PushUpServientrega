...
export class EmailDTO{
    ...
    @Expose ({name: 'email'})
    @IsString()
    @Transform(({value})=>{if(/^[a-z A-Z áéíóúÁÉÍÓÚñÑ 0-9]+$/.test(value)) return value; else throw {status:400, message: `El dato email incumple los parametros acordados`};}, {toClassOnly:true})
    email: string;
